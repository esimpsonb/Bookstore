...
class User():
    ...
    def user_information(self):
        ...
        cart_book_titles = [book.title for book in self.cart.added_books]
        print("username: "+self.username)
        print("money: "+str(self.money))
        print("books: "+''.join(map(str, book_titles)))
        print("cart: "+''.join(map(str, cart_book_titles)))
        
    
    def options(self):
        print([str(method) for method in dir(self) if callable(getattr(self, method)) and not method.startswith("__")])
